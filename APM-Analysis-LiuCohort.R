# cohortLiu_df: data frame of transcriptome (in TPM) & clinical data in the Liu cohort. 

# housekeeping gene score: for normalization
cohortLiu_df$ACTB_log=log2(cohortLiu_df$ACTB+1)
cohortLiu_df$GAPDH_log=log2(cohortLiu_df$GAPDH+1)
cohortLiu_df$SDHA_log=log2(cohortLiu_df$SDHA+1)
cohortLiu_df$HPRT1_log=log2(cohortLiu_df$HPRT1+1)
cohortLiu_df$HBS1L_log=log2(cohortLiu_df$HBS1L+1)
cohortLiu_df$AHSP_log=log2(cohortLiu_df$AHSP+1)
cohortLiu_df$housekeeping=(cohortLiu_df$ACTB_log+cohortLiu_df$GAPDH_log+cohortLiu_df$SDHA_log+cohortLiu_df$HPRT1_log+cohortLiu_df$HBS1L_log+cohortLiu_df$AHSP_log)/6
housekeeping_mean=mean(cohortLiu_df$housekeeping,na.rm=T)


# Fig 1a
features=c('PSME1','PSMB10','TAP2','B2M','CIITA','HLA_E','HLA_G','HLA_F','HLA_DRB6','HLA_DQA2','HLA_DQB2')
combo_vec=unlist(features)
df=subset(cohortLiu_df,select=(c('patientId','BOR',"progression_free","progression",'HLA_A','HLA_B','HLA_C',"HLA_DRA", "HLA_DRB1","HLA_DQA1","HLA_DQB1","HLA_DPA1","HLA_DPB1","HLA_DMA",'HLA_DMB',"HLA_DOA",'HLA_DOB',combo_vec)))
...
annotated_clusters=cutree(out_122$tree_row, k=num_clusters)
clusters_df=data.frame('orig'=annotated_clusters)
clusters_df$new=NA
clusters_df$new[clusters_df$orig==3]=1
clusters_df$new[clusters_df$orig==1]=2
clusters_df$new[clusters_df$orig==2]=3
clusters_df$new[clusters_df$orig==4]=4
...
annotation_row=annotation_row %>% arrange(annotation_row$Cluster)
...
out_122=pheatmap::pheatmap(data, color = my_palette, cluster_cols=T,cluster_rows=T, clustering_distance_rows = "euclidean",
                           clustering_method = 'complete', annotation_names_row = F,annotation_names_col = F, show_rownames = F,
                           legend=T, fontsize = 8, angle_col = "315", annotation_colors=annotation_colors,annotation_row=annotation_row,
                           main='Liu Melanoma anti-PD1')


num_clusters=4
cohortLiu_df$cluster=NA
pre_cluster=sort(cutree(out_122$tree_row, k=num_clusters))
for (i in (1:length(names(pre_cluster)))) {
  patientId=names(pre_cluster)[i]
  index=which(cohortLiu_df$patientId == patientId)
  if (length(index) == 0) {
    message('Not Found: ', patientId)
  }
  k=pre_cluster[i]
  # rename the clusters so the order is the same as their correlation to survival
  if (k == 3) {
    k=1
  } else if (k==1) {
    k=2
  ...
  cohortLiu_df$cluster[index]=k
}
cohortLiu_df$cluster=factor(cohortLiu_df$cluster)

cohortLiu_df$Cluster=paste('C',cohortLiu_df$cluster,sep='')
cohortLiu_df$Cluster=factor(cohortLiu_df$Cluster)
# Fig. 1c
fit <- survfit(Surv(progression_free,progression) ~ Cluster,data = cohortLiu_df)
# 430 * 550
ggsurvplot(fit, data = cohortLiu_df, palette = c('#CD9B1D',"#EAD6A3", "#BFB1D6",'#5E3C99'),
           conf.int = F, 
           pval = T,  
           risk.table = TRUE,
           xlab = '', ylab = 'Survival', 
           legend.labs = levels(factor(cohortLiu_df$Cluster)),
...
coxph(Surv(progression_free,progression) ~ Cluster, data = cohortLiu_df) %>% gtsummary::tbl_regression(exp = TRUE)


# Fig. 1e
# 420 * 500
facet_df=cohortLiu_df
...
ggplot(facet_df, aes(Cluster,fill=`clinical benefit`)) + 
  ...
  annotate(geom = "text", y = c(1,1,1,1) - 0.1, x = c(1,2,3,4)-0.2, label = c(C1_percent,C2_percent,C3_percent,C4_percent), size = 3, hjust = 0)
...
for (i in (1:length(genes))) {
  ggsave(plot_list[[i]], file=paste0(levels(factor(genes))[i],".png"), width = 10, height = 8, units = "cm")
}
...
# Fig. 2a
C1=subset(cohortLiu_df,cohortLiu_df$cluster == 1)
C2=subset(cohortLiu_df,cohortLiu_df$cluster == 2)
C3=subset(cohortLiu_df,cohortLiu_df$cluster == 3)
C4=subset(cohortLiu_df,cohortLiu_df$cluster == 4)
...
pheatmap::pheatmap(data, color = my_palette, cluster_cols=T,cluster_rows=F, annotation_colors = annotation_colors,
                   ...
                   border_color=NA, legend=T, angle_col = "315",main='Liu Melanoma anti-PD1')
...
# # Fig. 2g hazard ratio
cohortLiu_df$CYT=(log2(cohortLiu_df$GZMA + 1) + log2(cohortLiu_df$PRF1 + 1)) / 2
cohortLiu_df$highCYT=cohortLiu_df$CYT > unname(quantile(cohortLiu_df$CYT,na.rm=TRUE)["75%"])
cohortLiu_df$highTMB=cohortLiu_df$nonsyn_muts > 100
cohortLiu_df$C1=F
cohortLiu_df$C1[which(cohortLiu_df$cluster == 1)]=T
cohortLiu_df$C2=F
cohortLiu_df$C2[which(cohortLiu_df$cluster == 2)]=T
...
# IFN response
dat.m1 <- melt(cohortLiu_df %>% filter(!is.na(cluster)),
               id.vars='cluster', 
...
# 350 * 500
ggplot(cohortLiu_df, aes(x=Cluster, y=purity, fill=Cluster)) +
  geom_violin() +
  stat_compare_means(comparisons = my_comparisons, size=3) + 
...
dat.m1$Cluster[which(dat.m1$cluster == 4)]='C4'
# 350 * 500
cohortLiu_df$TMB=log10(cohortLiu_df$nonsyn_muts)
ggplot(cohortLiu_df, aes(x=Cluster, y=TMB, fill=Cluster)) +
  geom_violin() +
...
dat.m1 <- melt(cohortLiu_df %>% filter(!is.na(cluster)),
               id.vars='cluster', 
               measure.vars=c('SSGSEA_TGFB_SIGNALING','SSGSEA_HYPOXIA'))
dat.m1$label=NA
...
df=subset(cohortLiu_df,select=(c('patientId','BOR',"progression_free","progression",'B2M','HLA_A','HLA_B','HLA_C','HLA_E','HLA_G','HLA_F','CIITA',"HLA_DRA", "HLA_DRB1","HLA_DQA1","HLA_DQB1","HLA_DPA1","HLA_DPB1","HLA_DMA",'HLA_DMB',"HLA_DOA",'HLA_DOB')))
